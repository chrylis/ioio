		...
		public final void run() {
			...
			while (!abort_) {
				try {
					...
					LoggerProvider.INSTANCE.e(TAG, "Failed to create IOIO, aborting IOIOThread!");
					return;
				}
				...
				try {
					...
					LoggerProvider.INSTANCE.e(TAG, "Incompatible IOIO firmware", e);
					looper_.incompatible();
					// nothing to do - just wait until physical
					// disconnection
				} catch (Exception e) {
					LoggerProvider.INSTANCE.e(TAG, "Unexpected exception caught", e);
					ioio_.disconnect();
					break;
				} finally {
					try {
			...
			LoggerProvider.INSTANCE.d(TAG, "IOIOThread is exiting");
		}
